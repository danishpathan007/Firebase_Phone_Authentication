...
extension SkyFloatingLabelTextFieldWithIcon{
    func setRightViewIcon(icon: UIImage,viewController:UIViewController,action:Selector,tag:Int) {
        ...
        btnView.addTarget(viewController, action: action, for: .touchUpInside)
        self.rightView = btnView
        btnView.tag = tag
        let img = UIImageView()
        img.tag = tag
        img.frame = CGRect(x: btnView.center.x-8, y: btnView.center.y, width: 15, height: 15)
        img.image = icon
        btnView.addSubview(img)
    }
    
    func changeImage(icon:UIImage,tag:Int){
        if let subview = rightView{
            if let item = subview as? UIButton{
                for image in (item.subviews){
